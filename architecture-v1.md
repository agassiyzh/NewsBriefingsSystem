...
3. 引入启用 Hermes/Honcho memory 的 Editor-in-chief profile 作为长期编辑记忆所有者；新闻仓库程序只产出建议和证据，不直接写长期记忆，同时严格隔离 Editor 与 Reporter。
...
6. 月度分析由 Analyst 汇总 D1 反馈、简报历史和发布表现，生成 monthly insights 与 editorial preference recommendations；Editor 审核稳定偏好后，写入自己的 Hermes/Honcho memory。
7. Coder 最终把运行脚本、提示词、数据契约和操作文档沉淀为 `personal-newsroom` skill，供其他角色复用。

核心架构决策：v1 不构建复杂多服务系统，不引入队列，不引入独立后端数据库服务器；在现有最小闭环旁边增加结构化数据、发布目录、反馈 API 和月度分析闭环。新 runner 满足预检后可直接 cutover 到生产 cron，但必须保留旧 job 的 pause/rollback 证据，不删除旧链路。
...
9. 本文不直接执行 GitHub repo、Cloudflare 资源或 cron 修改；但用户已授权后续 ops 对真实 Cloudflare/Pages deploy 与生产 cron 做变更。新 runner 可在通过预检后直接 cutover，旧 cron/job 只 pause 不 delete，并保留 rollback 证据。
...
- `editor-memory-owner`：启用 Hermes/Honcho memory 的 Editor-in-chief profile，负责读取/写入长期编辑偏好；仓库内任何 Honcho apply adapter 仅可作为 deprecated 的本地迁移/debug 工具，不属于生产路径。
...
6. 读取 Analyst 的月度洞察和偏好建议，决定是否更新自己的 Hermes/Honcho 编辑记忆。
7. 审核可公开内容，避免泄露隐私或输出过度投资建议。

记忆边界：

- Editor-in-chief profile 是长期编辑记忆所有者，可以读取和写入自己的 Hermes/Honcho memory。
- Editor 记忆保存“偏好、模式、长期关注、写作风格、经过审核的反馈总结”，不保存每条原始新闻全文或 raw feedback events。
...
- Reporter 保持 stateless：默认不写入、也不直接读取长期 Honcho 编辑记忆。
- Reporter 只接收 Editor 本轮 brief，并可持有短期任务上下文：本轮采集 brief、已查来源、已排除条目。
...
- Analyst 不直接写入 Editor 长期记忆；必须由 Editor 审核后，由 Editor profile 自己写入。
...
- Publisher 不写 Hermes/Honcho 长期编辑记忆。
...
- Coder 不把实现日志、运行细节或自动化建议写入 Editor 长期偏好。
...
5. Analyst 输出 `monthly_insights`：
   ...
   - 编辑建议。
   - 置信度。
   - 可提交给 Editor 的 editorial preference recommendations。
6. Editor 审核 Analyst 建议，筛掉单月热点、低样本推断和任何 raw events。
7. Editor 将确认后的稳定偏好写入自己的 Hermes/Honcho memory；新闻仓库程序不直接写入长期编辑记忆。
...
Honcho 是 Editor-in-chief profile 拥有的长期编辑记忆，不是全文数据库、日志数据库或反馈明细库。
...
5. 月度分析后的编辑偏好
   - 经过 Analyst 分析并由 Editor 审核、确认稳定后的结论。
...
- Editor 从自己的 Hermes/Honcho memory 读取长期偏好。
- Editor 把本轮采集要求压缩成 Reporter brief。
- Reporter 只看到 brief，不直接访问完整 Honcho memory。
- Reporter 返回材料，不写 Honcho。
- Analyst 返回建议，不写 Honcho。
- 只有 Editor 审核后，才由 Editor profile 写入自己的 Hermes/Honcho memory。
...
CREATE TABLE IF NOT EXISTS monthly_insights (
  ...
  editor_memory_recommendation TEXT,
...
6. `editorial_preferences` 存已结构化的偏好建议和 Editor 审核状态；它不是生产 Honcho 写入通道，只可作为本地审计、debug 或迁移桥接表。
7. 命名兼容：历史字段或脚本中若仍出现 `honcho_update_proposal` / `apply_editorial_preferences`，只能理解为旧兼容或本地 debug，不代表生产写入路径；新文档和新实现应优先使用 `editor_memory_recommendation` / `recommendations`。
...
5. 保持 Editor profile 作为长期记忆所有者；即使未来 Analyst 月度任务迁移为 profile-owned cron，Analyst 也只提交建议，不直接写 Editor memory。
...
4. Analyst 输出建议。
5. Editor 审核后，由 Editor profile 写入自己的 Hermes/Honcho memory；`editorial_preferences` 仅作为本地审计/debug 记录。

验收标准：

- 能生成上月分析报告。
- 报告包含高兴趣主题、低兴趣主题、来源质量、格式建议。
- 不直接把原始行为日志写入 Honcho/Hermes memory。
...
- Analyst 输出置信度。
- 不因单次事件改变长期记忆。
- 只把经 Editor 审核的月度稳定模式写入 Editor-owned Honcho/Hermes memory。
...
14. Editor memory 更新协议
   - 写 `docs/honcho-memory-policy.md` 或放入 skill。
   - 明确 Editor profile owns memory；Analyst 只提交 recommendation；新闻 repo 的 apply adapter 不是生产写入路径。
...
- 如何查询和分析反馈。
- 如何由 Editor profile 安全更新 Hermes/Honcho 记忆。
...
6. ID 规范。
7. 发布安全规则。
8. Editor-owned Hermes/Honcho 记忆规则。
9. 反馈分析规则。
10. 禁止事项。
...
1. Memory Policy
   - 什么能进入 Editor-owned Hermes/Honcho memory。
   - 什么不能写入长期记忆。
   - Analyst -> Editor recommendation 审核流程。
...
memory:
  provider: hermes_honcho
  owner_profile: editor-in-chief
  repo_programmatic_apply: false
  editor_can_write: true
  reporter_can_write: false
  analyst_can_write: false
  publisher_can_write: false
  coder_can_write: false
```
...
7. v1 Honcho/Hermes memory 由 Editor-in-chief profile 拥有，只存长期编辑偏好，不存原始事件和敏感数据。
8. v1 Editor 与 Reporter 记忆隔离；Reporter stateless，只接收本轮 brief。
9. v1 月度分析结果必须由 Editor 审核后，才由 Editor profile 写入长期记忆；新闻 repo 不使用 endpoint/token/apply flag 作为生产写入路径。
10. v1 所有新增能力必须可关闭；新 runner 可直接 cutover 到生产 cron，但必须保留旧 job pause/rollback 证据，不破坏 Telegram + Markdown 最小闭环。
